...
This is very close, but there is one additional step. The expression above will work in a Python REPL, but it will not work within an `eval`. The generator will not be able to resolve the `gen` variable when it executes because of subtlety in how variables are resolved within `eval`, explained [here](https://docs.python.org/3/reference/executionmodel.html#interaction-with-dynamic-features).

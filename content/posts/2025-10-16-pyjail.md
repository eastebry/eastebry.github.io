...
I wouldn't bet on `asteval` being completely bulletproof. Sandboxing python is notoriously difficult - although `asteval`'s strategy is a much smarter way to go about it, it still feels like betting against the house. `asteval` also seems to include `numpy` functions by default, which greatly increases the attack surface (thankfully this is disabled in Checkov's invocation.)
